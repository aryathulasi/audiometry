...
if( $go ){
    ...
    $sql = 'insert into t_test( uid, ear ,date)values(:uid,:ear ,:date)';
			$params = array(':uid'=>$uid,':ear'=>$ear,':date'=> Date('Y-m-d'));
    ...
    $params = array(':ear'=>  $ear,':testid'=>  $_SESSION['t_l_id']); 
...
if( $go ){
    ...
    $sql = 'update t_test set ear=:ear where testid=:testid';
    $params = array(':ear'=>  $ear,':testid'=>  $_SESSION['t_r_id']); 
    $result = $db->execute_query( $sql, $params );
